...
export function handleFirebaseAuthError(error: unknown): AuthErrorResponse {
  if (error && typeof error === 'object') { // First check if error is an object
    const firebaseError = error as { code?: string; message?: string }; // Check for FirebaseError shape instead of instanceof
    ...
    if (firebaseError.code) {
      switch (firebaseError.code) {
        ...
        default:
          return { 
            success: false, 
            message: `Authentication error: ${firebaseError.message}`, code: "INTERNAL_ERROR" };
      }
    }
    
    // If we have a message but no code, return it with internal error
...
export function getErrorAlertVariant(errorCode: ErrorCode | undefined) {
